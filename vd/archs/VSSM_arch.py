...
class VSSBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.self_attention = SS2D(d_model=hidden_dim, d_state=d_state, expand=2, dropout=attn_drop_rate, **kwargs)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm1 = norm_layer(hidden_dim)
        self.norm2 = norm_layer(hidden_dim)
        mlp_hidden_dim = int(hidden_dim * 4)
        self.mlp1 = MLP(in_features=hidden_dim, hidden_features=mlp_hidden_dim, out_features=hidden_dim, act_layer=nn.GELU)

        self.gamma = nn.Parameter(layerscale_value * torch.ones((hidden_dim)), requires_grad=True)

    def forward(self, input, x_size):
        H , W = x_size
        B, L, C = input.shape
        x = input.view(B, *x_size, C).contiguous()  
        x = x + self.drop_path(self.self_attention(self.norm1(x)))
        x = x.view(B, -1, C).contiguous()
        x = x + self.drop_path(self.mlp1(self.norm2(x), H, W))  

        x = x + (input * self.gamma)
...
class ResidualGroup(nn.Module):
    ...
    def __init__(self,
        ...
        self.sigma = nn.Parameter(layerscale_value * torch.ones((dim)), requires_grad=True)

    def forward(self, x, x_size):

        xx = self.residual_group(x, x_size)
        xx = self.patch_unembed(xx, x_size)
        xx = self.conv(xx)
        xx = self.patch_embed(xx) + (x * self.sigma)
...
class VSSM(nn.Module):
    def __init__(self,
        ...
        self.conv_second = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)
